...
import '../../../sms_listener/presentation/bloc/sms_listener_bloc.dart';
import '../../../sms_listener/presentation/bloc/sms_listener_event.dart';
import '../../../sms_listener/presentation/bloc/sms_listener_state.dart';
import '../../../sms_listener/presentation/widgets/sms_settings_tile.dart';
import '../../../sms_listener/presentation/widgets/transaction_review_dialog.dart';
...
class _ExpenseDashboardScreenState extends State<ExpenseDashboardScreen> {
  ...
  void initState() {
    ...
    context.read<SmsListenerBloc>().add(const InitializeSmsListener());
  }
  ...
  void _showTransactionReviewDialog(BuildContext context, dynamic parsedExpense) {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (dialogCtx) => TransactionReviewDialog(
        ...
        onConfirm: () {
          ...
          context.read<ExpenseBloc>().add(AddExpenseSubmitted(expense));
          context.read<SmsListenerBloc>().add(const ClearDetectedTransaction());
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('✨ Auto-detected expense confirmed & saved!'),
    ...
    return Scaffold(
      ...
      body: MultiBlocListener(
        listeners: [
          ...
          BlocListener<SmsListenerBloc, SmsListenerState>(
            listener: (context, state) {
              if (state is TransactionDetected) {
                _showTransactionReviewDialog(context, state.parsedExpense);
              }
            },
          ),
        ],
